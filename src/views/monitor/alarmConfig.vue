  ...
  >
    ...
    <template #tableDefinedSlot="slotProps">
      <div v-if="slotProps.prop === 'operator'">
        <el-link type="primary" @click="openUpdateDialog(slotProps.scope.row)"
                 href="javascript:;">更新
        </el-link>
      </div>
    </template>
    ...
    <template #dialog>
      ...
      <!-- 更新对话框 -->
      <compo-dialog :dialogParams="updateDialog" ref="updateDialogRef" @confirmSuccess="updateSuccess"></compo-dialog>
    </template>
...
const addForm = [
  ...
  {
    label: '检查项', prop: 'checkField', type: 'select', rules: true, fixedSpan: 12,
    config: {options: fieldOptions},
  },
  {
    label: '监控范围', prop: 'monitorRange', type: 'select', rules: true, fixedSpan: 12,
    config: {options: monitorRangeOptions},
  },
  {
    label: '运算符', prop: 'thresholdOperator', type: 'select', rules: true, fixedSpan: 12,
    config: {options: operatorOptions},
  },
  {
    label: '阈值', prop: 'highThreshold', type: 'input', rules: true, fixedSpan: 12,
